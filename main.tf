resource "aws_secretsmanager_secret" "sm" {
  ...
  name                           = try(each.value["tf_outputs"], false) ? "/${try(each.value["secret_name_override"], each.key)}/${var.region}/${var.environment}/${each.value["cluster_name"]}/values.yaml" : (lookup(each.value, "name_prefix", null) == null ? each.key : null)
